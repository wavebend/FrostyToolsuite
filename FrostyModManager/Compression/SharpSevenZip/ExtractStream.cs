...
namespace SharpSevenZip
{
    internal class ExtractStream : Stream
    {
        ...
        private readonly Queue<IMemoryOwner<byte>> _data = new Queue<IMemoryOwner<byte>>();
        private readonly Queue<int> _dataLength = new Queue<int>();
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (readCount < count)
            {
                ...
                _currentData.Memory.Span.Slice(_position, currentCount).CopyTo(buffer.AsSpan().Slice(offset + readCount));
