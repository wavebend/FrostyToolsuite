...
namespace SharpSevenZip
{
    ...
    public abstract class SharpSevenZipBase : MarshalByRefObject
    {
        ...
        internal void CheckedExecute(int hresult, string message, CallbackBase handler)
        {
            if (hresult != (int)OperationResult.Ok || handler.HasExceptions)
            {
                if (!handler.HasExceptions)
                {
                    ...
                    {
                        SharpSevenZipException exception;

                        switch (hresult)
                        {
                            case -2146233067:
                                exception = new SharpSevenZipException("Operation is not supported. (0x80131515: E_NOTSUPPORTED)");
                                break;
                            case -2147024784:
                                exception = new SharpSevenZipException("There is not enough space on the disk. (0x80070070: ERROR_DISK_FULL)");
                                break;
                            case -2147024864:
                                exception = new SharpSevenZipException("The file is being used by another process. (0x80070020: ERROR_SHARING_VIOLATION)");
                                break;
                            case -2147024882:
                                exception = new SharpSevenZipException("There is not enough memory (RAM). (0x8007000E: E_OUTOFMEMORY)");
                                break;
                            case -2147024809:
                                exception = new SharpSevenZipException("Invalid arguments provided. (0x80070057: E_INVALIDARG)");
                                break;
                            case -2147467263:
                                exception = new SharpSevenZipException("Functionality not implemented. (0x80004001: E_NOTIMPL)");
                                break;
                            case -2147024891:
                                exception = new SharpSevenZipException("Access is denied. (0x80070005: E_ACCESSDENIED)");
                                break;
                            case -2146233086:
                                exception = new SharpSevenZipException("Access is denied. (0x80070005: E_ACCESSDENIED)");
                                break;
                            case -2147024690:
                                exception = new SharpSevenZipException("Argument is out of range. (0x80131502: E_ARGUMENTOUTOFRANGE)");
                                break;
                            default:
                                exception = new SharpSevenZipException(
        ...
        {
            if (!(obj is SharpSevenZipBase instance))
            {
                return false;
            }
